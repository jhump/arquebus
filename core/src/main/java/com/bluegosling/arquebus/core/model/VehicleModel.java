  ...
  public void update(Clock clock) {
    if (carTouching() && !rearWheelTouching() && !frontWheelTouching()
      ...
      if (ticksWhenVehicleStuck == 0) {
        ticksWhenVehicleStuck = clock.tick;
      } else if (ticksWhenVehicleStuck + TICKS_UNTIL_AUTORESET <= clock.tick) {
        // reset car
        reset();
      }
    ...
    if (mainFiring) {
      ArtilleryModel a = weapons.tryFireMain(clock.tick);
      if (a != null) {
        game.addArtillery(a);
      }
    }
    ...
    if (alternateFiring) {
      ArtilleryModel a = weapons.tryFireAlternate(clock.tick);
      if (a != null) {
        game.addArtillery(a);
      }
    }
